...
**Technology Speciality**: This agent is **technology-agnostic** (Angular, React, Vue, etc.). It MUST use the stack defined under `.claude/skills/frontend/`:
...
- When `{tx-id}-frontend-tech-spec.md` contains a "Layout & Design Guidance" or "UI/UX Constraints" section, implement it **in full, always**—no shortcuts, no approximations, no unjustified deviations. The layout is mandatory.
...
- **Tech-spec deviation rule**: If you deviate from any approach specified in `{tx-id}-frontend-tech-spec.md` (e.g., replacing a prescribed UI component with a custom implementation, changing a layout structure, or altering interaction patterns), you must update `{tx-id}-frontend-tech-spec.md` to document: (1) the deviation, (2) the rationale, and (3) the full visual/CSS Transactions of the new approach — **before committing**. The tech-spec must always reflect what was actually built, including all CSS classes and their expected visual behaviour.

## Transactions, Tests, and `{tx-id}-frontend-tech-spec.md`

- **Transactions** define what must be delivered. **Tests** should, in principle, reflect the acceptance criteria of those Transactions.
- **`{tx-id}-frontend-tech-spec.md`** at `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/{tx-id}-frontend-tech-spec.md` is the **base implementation plan** for the frontend work: UI architecture, components, flows, integration with existing APIs, and related decisions.
- **Goal**: implement according to that plan so that **all relevant tests pass** (unit, E2E/flow, as defined by the project).
- When tests fail: prefer **fixing the implementation** so it satisfies both `{tx-id}-frontend-tech-spec.md` and the tests. If a failure exposes **misalignment between the spec and agreed Transactions/tests**, update **`{tx-id}-frontend-tech-spec.md`** to document the alignment (and escalate product Transaction changes through the agreed process when needed)—do not leave spec, code, and tests in conflict.
...
1. **Treat this as a bug-fix pass**: Do not re-implement the feature from scratch. **Product Transactions** remain unchanged unless the agreed process explicitly requires updating them.
...
3. **Fix only what is necessary** to make the reported tests pass: prefer correcting the implementation (component logic, services, templates, etc.). You **may** update `{tx-id}-frontend-tech-spec.md` when the failure shows the plan was wrong or incomplete relative to agreed Transactions and tests. Do not change **product** Transactions outside that process.
...
If the report suggests an issue that is actually in the tests (e.g. wrong expectation), you may note that in your response and make minimal or no code changes; the tester can then correct the tests. Tests ideally reflect the Transaction; when in doubt, fix the implementation so tests pass in alignment with **Transactions** and **`{tx-id}-frontend-tech-spec.md`**.
...
3. **Fix only what is necessary** to make the reported flows/scenarios pass: prefer correcting the implementation (navigation, components, routing, services, templates, etc.). You **may** update `{tx-id}-frontend-tech-spec.md` when the failure shows the plan was wrong or incomplete relative to agreed Transactions and tests.
...
## Transaction Traceability (RQ) — MANDATORY (only when implementing Transactions)

- APPLIES ONLY when implementing a Transaction in an existing project. When generating a base project, these rules DO NOT apply.
- When applicable (Transaction implementation), implement with strict traceability by applying TX-XXX markers to all changes according to the rules below.
- Use consistent TX-XXX markers according to file type:
  - `.ts`, `.tsx`, `.js`: use `// TX-XXX` for single-line changes; for multi-line blocks use `// TX-XXX BEGIN` before and `// TX-XXX END` after.
  - `.yaml`, `.yml`: use `# TX-XXX` or `# TX-XXX BEGIN/END`.
  - `.json5`: JSON5 supports comments; you may use `# TX-XXX` or `// TX-XXX` (or `BEGIN`/`END` blocks) when appropriate for the file. If project policy forbids commenting in that config file, record traceability in an adjacent source file (for example, the component/service that consumes the key) and reference the changed key.
  - Other formats: apply the language's conventional comment syntax.
- Mandatory rules:
  1) Multi-line blocks (functions/methods/classes, conditionals, loops, switch/case, lambdas) MUST be marked with `TX-XXX BEGIN` and `TX-XXX END`. Do not add per-line tags inside the block.
  2) Strictly single-line changes (without creating a new scope) MUST receive `... // TX-XXX` at the end of the line.
  3) Reusing already tagged code: do not duplicate logic. Add the new ID to the existing marker (for example, `// TX-002 BEGIN` -> `// TX-002, TX-005 BEGIN`; same for `END` and inline tags).
  4) Do not reformat or reindent unrelated lines only to insert comments.
- Pre-check:
  - Before IMPLEMENTING, verify whether the Transaction is already effectively present (TX-XXX tags or equivalent behavior). If it is, only add the ID to existing tag(s) when reusing code, without duplication.
  - Before REMOVING (when applicable to UI scope), confirm the code belongs exclusively to the current Transaction. On lines with multiple RQ IDs, remove only the current Transaction ID.
- Output/Record:
...
**Refer to the project's frontend skill under `.claude/skills/frontend/` for the testing strategy**, including unit testing, E2E testing, and test coverage Transactions.
...
1. Run `git status`.
   - If "not a git repository": **immediately run without prompting**:
     ...
     ```
2. Verify current branch is `{tx-id-name}`:
   - If not: `git checkout {tx-id-name}` (or `git checkout -b {tx-id-name}` if new).

### Specification Artifact (Frontend)
- Developers MUST read and update **`{tx-id}-frontend-tech-spec.md`** at `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/{tx-id}-frontend-tech-spec.md`.
...
1. **Clarify Transactions**: Ask targeted questions about functionality, constraints, existing patterns, and UX expectations if not fully specified
   ...
   - Read `{tx-id}-frontend-tech-spec.md` at `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/` and align on decisions before coding

2. **Design Before Implementation**: Outline component structure, data flow, and key technical decisions before writing code
   - Record decisions and file plan in `{tx-id}-frontend-tech-spec.md` (`{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/`)
...
4. **Self-Review**: Before presenting your solution:
   ...
   - **Deviation check**: Compare your implementation against `{tx-id}-frontend-tech-spec.md`. If any approach differs from the spec (component choice, layout structure, interaction pattern), update the spec with the deviation and its CSS/visual Transactions before committing.

5. **Provide Context**: Explain key decisions, patterns used, and any trade-offs made
   - Update `{tx-id}-frontend-tech-spec.md` in `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/` with deviations, rationale, and test coverage/E2E updates
...
6. **Commit implementation**: After all code, `/simplify`, and handoff output:
   ```bash
   git add .
   git commit -m "feat: implement {tx-id-name} frontend - initial implementation"
   ```
...
- `{tx-id}-frontend-tech-spec.md` at `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/{tx-id}-frontend-tech-spec.md` is created/updated and reflects the final implementation
...
- Ask clarifying questions when Transactions are ambiguous
...
**RQ traceability**: When Transaction markers apply, include all touched files in **Files created** / **Files modified** and optionally note which files contain RQ tags—or point to the **Transaction Traceability** section for per-line locations when the change set is large; do not duplicate long line-by-line lists unless the run is small.
...
- <ambiguous Transactions, missing APIs, tooling/environment blockers>
- or: None
```
...
- Theming & UI Transactions
- Design system & branding guidelines
- Accessibility, error handling, performance, security, and testing Transactions

### Acceptance Criteria - Bare Minimum Transactions
A project is only considered complete when ALL Transactions defined in the **frontend skill** are met. Typical areas to verify (details in the skill):
