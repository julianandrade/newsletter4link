...
4. **Identify `{tx-id-name}`**: The full Transaction folder name under `{{PATH_DOCS}}/4-implementation/development/` (e.g. `TX-001-criar-tarefa`); use it in all paths. Extract from the provided documentation file path or ask the user if not clear. The short **Transaction ID** (e.g. `TX-001`) is used in the "Transaction" field of reports where applicable.
5. **Create Directory Structure**: Ensure the following directory structure exists under `{{PATH_DOCS}}/` (do not place flow artifacts under application source code—use this path only), creating it if necessary:
   - `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/`
   - `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/screenshots/`

## Phase 2: Execute Flow Tests

**Headful mode**: Launch the browser in **headed (non-headless) mode** so the browser window is visible during test execution. Configure Playwright MCP accordingly before running any flow.
...
5. **Layout validation** (when `{tx-id}-frontend-tech-spec.md` exists with "Layout & Design Guidance" or "UI/UX Constraints"): Compare the visible layout (structure, spacing, components) with the spec. If layout does not match → record as **FAILED** with error "Layout non-compliance: [specific deviation from spec]".
...
- Save the report as `Flow_Test_Report_[TIMESTAMP].md` in **`{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/`**. All report content and artifact names must be in **English**.
- Take screenshots and snapshots for each screen at **viewport size** (visible browser window), NOT full page. Save them in **`{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/screenshots/`**. Verify paths before finalizing.
...
Include flow failures and **layout non-compliance** (layout does not match `{tx-id}-frontend-tech-spec.md`).
...
Save this report (e.g. as `E2E_Flow_Failure_Report_[TIMESTAMP].md`) in **`{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/`** only, or include it in full in your response. All content must be in **English**. Explicitly say: "The developer agent should be re-invoked with the E2E/Flow Failure Report above to fix the implementation. After they commit fixes, re-run unit tests (7a), then build (7a2), then flow-test and robot-tester (7b)."

## Mandatory completion output (handoff)

At the end of every substantive run, you **must** emit this structured handoff in **English**. Detailed flow tables belong in **Flow_Test_Report_[TIMESTAMP].md** and **E2E_Flow_Failure_Report_[TIMESTAMP].md**; the handoff **references paths** under `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/` and `flows/screenshots/` instead of duplicating full tables.
...
## Files created
- <{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/flows/Flow_Test_Report_*.md>
