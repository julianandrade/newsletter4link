...
Use this skill when you need to **generate** unit tests (TDD mode — step **5d**) or **run and validate** existing unit tests (validation mode — step **7a**) in **`/frontend-development`** or **`/backend-development`**. **Skip** when `features.test` is `false` in `settings.json`.
...
| Mode | Step | Purpose |
|------|------|---------|
| **TDD** | 5d | Generate unit tests from tech-spec and complete-transaction. No code exists; tests will fail when run (Red phase). |
...
| Context | How to run |
|---------|------------|
| **Direct** (manual) | Invoke with tx-id and context (code for legacy mode, or tech-spec for TDD mode). Prefer launching **unit-test-generator** (`.claude/agents/tests/unit-test-generator.md`) with this skill's context. If unavailable, main agent executes the procedure. |
| **In flow** | Step 5d invokes unit-test-generator in TDD mode; step 7a invokes in validation mode. |

## Purpose

- **TDD mode (5d)**: Generate unit tests from tech-spec and complete-transaction. Tests are written in the project under test; when run, they **must fail** (Red phase) because no implementation exists. Output feeds step 6 (Developer).
...
- **TDD mode (5d)**: Tech-spec, `{tx-id}-complete-transaction.md`, Transaction folder.
- **Validation mode (7a)**: Feature code (from Developer), unit test files (from 5d), tech-spec, Transaction folder.

## Process

### TDD mode (step 5d)

1. **Generate tests**: Create unit tests from tech-spec and complete-transaction. Do **not** analyze existing code; infer contracts and behavior from the specification.
...
3. **Report location**: `{{PATH_DOCS}}/4-implementation/development/{tx-id-name}/tests/unit-test-reports/Test_Failure_Report_[TIMESTAMP].md` (never in project under test).
