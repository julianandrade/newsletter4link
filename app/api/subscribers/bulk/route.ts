...
/**
 * PATCH /api/subscribers/bulk
 *
 * Resubscribe, unsubscribe or delete many subscribers in one statement.
 *
...
export async function PATCH(request: Request) {
  try {
    ...
    if (unique.length > MAX_IDS) {
      return NextResponse.json(
        { error: `Cannot act on more than ${MAX_IDS} subscribers at once` },
        { status: 400 }
      );
    }
    ...
    console.error("Error applying bulk subscriber action:", error);
