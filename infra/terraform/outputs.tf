...
output "egress_ip" {
  description = <<-EOT
    The reserved static egress address. Every outbound request from Cloud Run leaves from
    here. This is the value to send to beehiiv's side for allowlisting, and the value to
    compare against what an IP echo service reports from inside a Cloud Run request:
    matching is the only proof the ALL_TRAFFIC egress setting took effect.
  EOT
